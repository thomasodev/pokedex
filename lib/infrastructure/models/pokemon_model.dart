...
class PokemonModel {
  ...
  PokemonEntity toEntity() => PokemonEntity().copyWith(
        name: name,
        image: sprites?.other?.officialArtwork?.frontDefault,
        abilities: abilities
            ?.map((Abilities abilities) => abilities.ability?.name ?? '')
            .toList(),
        moves: moves?.map((Moves moves) => moves.move?.name ?? '').toList(),
        types: types?.map((Types types) => types.type?.name ?? '').toList(),
        stats: stats
            ?.map(
              (Stats stats) => StatsEntity().copyWith(
...
class PastTypes {
  ...
  PastTypes.fromJson(Map<String, dynamic> json) {
    ...
    if (json['types'] != null) {
      ...
      json['types'].forEach((v) {
        types!.add(TypesGeneration.fromJson(v));
      });
    }
  }
...
class TypesGeneration {
  ...
  TypesGeneration({this.slot, this.type});

  TypesGeneration.fromJson(Map<String, dynamic> json) {
    slot = json['slot'];
